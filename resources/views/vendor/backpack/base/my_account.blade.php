...
@section('content')
    <div class="row">
        ...
        <div class="col-lg-8">
            <form class="form" action="{{ route('account.newtoken', auth()->user()) }}" method="post">
                ...
                <div class="card padding-10">
                        ...
                        <ul class="list-group">
                            ...
                            @foreach(auth()->user()->tokens as $token)
                                <li class="list-group-item">
                                    ...
                                    <div class="col-md-4">
                                        <label>{{ $token->created_at }}</label>
                                    </div>
                                    <div class="col-md-4">
                                        <label>{{ $token->last_used_at }}</label>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="card-footer">
                        <div class="col-md-4 form-group">
                            <label class="required">New Token Name</label>
                            <input required class="form-control" type="text" name="token_name" id="token_name" value="{{ session('token_name') }}">
                        </div>
                            <label>Plain Text Value</label>
                            <p>{{ session('token') ?? '' }}</p>
                        <button type="submit" class="btn btn-success"><i class="la la-save"></i> Create New Token</button>
                        <a href="{{ backpack_url() }}" class="btn">{{ trans('backpack::base.cancel') }}</a>
                    </div>
                </div>
