...
class DeployXlsForm implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // On local env, skip real deployment if the DEPLOYMENT_ENABLED is set to false
        if(config('app.env') === "local" && ! config('app.deployment_enabled')) {
            Log::info('Real deployment skipped for form' . $this->xlsform->name);
        ...
        if (collect($projects)->doesntContain($this->xlsform->project_name)) {
            ...
            $projectResponse = Http::withHeaders([
                ...
                ->post(
                    config('app.auth_url') . '/api/projects/create',
                    [
        ...
        $xlsformDataResponse = Http::withHeaders([
            ...
            ->post(config('app.auth_url') . '/api/forms/save', [
